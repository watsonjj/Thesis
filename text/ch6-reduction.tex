...
Contrary to the other techniques described in this section, ASTRI took the alternative direction of a hardware-implemented charge extraction, utilising their \gls{citiroc} \gls{asic}. The pulse from their \gls{sipm} is amplified and shaped (with both a high-gain and low-gain channel) with a constant shaping time of \SI{37.5}{ns}. The maximum of the shaped peak-height is then converted into an integrated charge, achieving less than \SI{1}{\percent} introduced systematic error.

The pulse from their \gls{sipm} is slow-shaped 

The \gls{fee} adopted for the ASTRI telescope prototype utilise the 

is the Cherenkov Imaging Telescope Integrated Read Out Chip (CITIROC [16]), INAF design intellectual property

Using their CITIROC \gls{asci}
