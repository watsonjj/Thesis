...
A common expression for the variation in photosensor response to a single photoelectron is its \glsf{enf}. This factor encompasses the multiplicative errors in the amplification process in both the \gls{mapmt} and \gls{sipmt}. In the definition used in this thesis, a perfect \gls{enf} is equal to 1. 

The dominating contributions to the \gls{enf} in a \gls{pmt} are the ``fluctuations in the secondary multiplication factor at each dynode (statistical fluctuation due to cascade multiplication), non-uniformity of multiplication depending on the dynode position and electrons deviating from their favourable trajectories'' \cite[][p. 127]{HAMAMATSU2007}. Conversely, the multiplication of charge in an \gls{sipmt} is very quantised in an \gls{sipmt} due to the microcells being operated in Geiger mode, and therefore functioning as binary device. This would suggest the \gls{enf} of an \gls{sipmt} is very close to 1. However, due to the statistical fluctuations caused by the optical crosstalk and afterpulsing of the device, the \gls{enf} is not perfect, and has the potential of being worse than that of a \gls{pmt}, despite the extremely high photoelectron counting resolution.
