...
Due to the dual-mirror Schwarzschild-Couder design of the \glsf{gct}, a compact camera design is possible\change{yawn}. The camera that has been developed for \gls{gct} is hence known as the \glsf{chec}. In order to enable this compactness, while making no compromise to image resolution of the shower\change{need to understand pixel angular size, and the minimum needed to resolve the image} 
...
\notes{see vinigrov for statement about enf, and how it could be worse for silicon even with the ``ultra- low excess noise of a limited Geiger mode avalanche multiplication'' due to opct}
