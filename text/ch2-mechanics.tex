...
HEGRA \gls{iact} on the Canary Island La Palma, the 1440 pixel \gls{sipmt} camera is installed in combination with a \SI{9.5}{m\squared} single mirror. As reported by \textcite{Biland2015}, \gls{sipmt} technology has been proved by \gls{fact} to be a viable alternative to \glspl{pmt} for future \glspl{iact}. This conclusion was reached even with the first generation of commercially available \glspl{sipmt}. \glspl{sipmt} have gone through considerable improvements since the construction of \gls{fact}.

The \glspl{sipmt} currently used in the \gls{chec-s} prototype are the Hamamatsu \change{continue}  

\subsection{Performance Parameters of SiPMs}

For an investigation into the performance of the \gls{chec-s} camera it is important 

\subsection{Gain}

As the charge read out from a \gls{sipmt} microcell is fixed by the quenching resistor, the gain of an \gls{sipmt} is characterised with the following simple relation between the capacitance of microcell diode $C$ and the overvoltage $\Delta V$ applied \cite{SensL2011}:
...
\end{equation}
where $e$ is the electron charge, $V_{bias}$ is the bias voltage, and $V_{br}$ is the breakdown voltage. 
...
\end{equation}
As indicated in Equation~\ref{eq:sipmt_pde}, and Figure~\ref{fig:sipm_pde}, the \gls{pde} of an \gls{sipmt} is a function of its overvoltage. As the overvoltage is increased, $\epsilon(V)$ approaches 1, and the \gls{pde} saturates.
