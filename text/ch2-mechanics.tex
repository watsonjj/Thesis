...
The waveforms per channel are delivered to the \gls{target} \gls{fpga}, where they are packaged into a \gls{udp} packet, and sent to the DACQ boards. The packets from each pixel are then combined, before they are sent to the camera server PC for storage.