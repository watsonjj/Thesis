...
Although there is no \gls{cta} requirements attached directly to it, the pulse shape of the camera is important to understand. The camera's pulse shape behaviour has an influence on its response during charge extraction, especially when using methods that utilise the expected shape of the pulse, such as a fitting technique, or the \textit{Cross Correlation} method. Figure
