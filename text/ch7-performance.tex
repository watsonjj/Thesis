...
Due to the dependence of the optical crosstalk on the bias voltage across the \gls{sipmt} \change{check with rich on parameter dependencies}, we were able to investigate the impact a reduced optical crosstalk has on \textit{Charge Resolution} with \textit{Lab} data. Three datasets were generated, each gain matched to a different ADC value (the transfer function were not included in the gain matching procedure at the time). A lower bias voltage is required to obtain a smaller gain-matching setting. Figure~\ref{fig:cr_4_opct_biasvoltage} shows the improvement in \textit{Charge Resolution} with reduced bias voltage, bringing the result lower than the requirement. However, the reduction in bias voltage also results in a decrease of Photon Detection Efficiency (PDE), which would result in the reduction in the camera's ability to resolve Cherenkov showers. This contradiction between meeting \textit{Charge Resolution} requirements, but reducing Cherenkov shower resolution, is one of the primary reasons the \gls{cta} requirements are being redefined to be in terms of photons, as described in Chapter~\ref{ch3-architecture}.

\subsection{Conclusion}

As suggested by the investigations in this sections, a reduction in optical crosstalk could allow \gls{chec-s} to meet the \textit{Charge Resolution} requirement in all calculation procedures.
